...
[Trait("Category", "Unit")]
public class ClearUserOverrideHandlerTests
{
    ...
    private readonly ClearUserOverride.Handler _sut;
    ...
    public ClearUserOverrideHandlerTests()
    {
        ...
        _sut = new ClearUserOverride.Handler(_featureReadDac, _featureWriteDac, _userReadDac, _featureGate);
    }
    ...
    private UserReadModel CreateTestUser() =>
        new(TestUserId, "Test User", "test@example.com", null, true,
            false, false, null, null, null, null, DateTime.UtcNow, null, DateTime.UtcNow, null);

    [Fact]
    public async Task Handle_ShouldReturnFail_WhenFeatureNotFound()
    {
        // Arrange
        var command = new ClearUserOverride.Command(TestUserId, TestFeatureCode);
    ...
    public async Task Handle_ShouldReturnOkAndInvalidateCache_WhenValid()
    {
        ...
        await _featureWriteDac.Received(1).ClearUserOverrideAsync(
