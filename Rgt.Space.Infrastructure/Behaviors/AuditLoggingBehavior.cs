    ...
    where TRequest : IRequest<TResponse>
{
    ...
    private static int DetermineStatusCodeFromException(Exception exception)
    {
        ...
        if (exception is Rgt.Space.Core.Errors.AppException appEx)
        {
            return Rgt.Space.Core.Errors.ErrorCatalog.GetStatusCode(appEx.ErrorCode);
        }
    ...
    private static int DetermineStatusCodeFromErrorCode(string errorCode)
    {
        ...
        {
            "SALE_NOT_FOUND" => Rgt.Space.Core.Constants.HttpConstants.StatusCodes.NotFound,
            "VALIDATION_ERROR" => Rgt.Space.Core.Constants.HttpConstants.StatusCodes.BadRequest,
            "TENANT_NOT_FOUND" => Rgt.Space.Core.Constants.HttpConstants.StatusCodes.NotFound,
            "UNAUTHORIZED" => Rgt.Space.Core.Constants.HttpConstants.StatusCodes.Unauthorized,
            "FORBIDDEN" => Rgt.Space.Core.Constants.HttpConstants.StatusCodes.Forbidden,
            _ => Rgt.Space.Core.Constants.HttpConstants.StatusCodes.InternalServerError
        };
    }
}
