...
public sealed class UserWriteDac : IUserWriteDac
{
    ...
    public async Task DeleteAsync(Guid userId, Guid deletedBy, CancellationToken ct = default)
    {
        ...
        await _pipeline.ExecuteAsync(async token =>
        {
            ...
            var cmd = new CommandDefinition(sql, new { Id = userId, DeletedBy = deletedBy }, cancellationToken: token);
            await conn.ExecuteAsync(cmd);
        }, ct);
    }
    ...
    public async Task<int> DeleteUserAssignmentsAsync(Guid userId, Guid deletedBy, CancellationToken ct = default)
    {
        ...
        return await _pipeline.ExecuteAsync(async token =>
        {
            ...
            var cmd = new CommandDefinition(sql, new { UserId = userId, DeletedBy = deletedBy }, cancellationToken: token);
            var rowsAffected = await conn.ExecuteAsync(cmd);
            return rowsAffected;
        }, ct);
    }
