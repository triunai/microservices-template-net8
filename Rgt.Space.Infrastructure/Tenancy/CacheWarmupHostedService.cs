...
public sealed class CacheWarmupHostedService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning("TenantMaster connection string not configured - skipping cache warmup");
                return;
            }
            ...
            if (!tenantList.Any())
            {
                _logger.LogWarning("No active tenants found in TenantMaster - nothing to warm");
                return;
            }
